...
class LeaveEntitlementDao extends BaseDao {
    ...
    public function getAnterieurEntitlements($empNumber, $leaveTypeId, $fromDate, $toDate) {
        try {
            $params = array(':leaveTypeId' => $leaveTypeId,
                ':empNumber' => $empNumber,
                ':fromDate' => $fromDate,
     ...
     * @return bool|string
     */
    public function getAnniversaireStart($empnumber)
    {
        $emp= $this->getEmployeeService()->getEmployee($empnumber);
        $startmois= new DateTime($emp->getDatedebut());
        $todaydate=new DateTime();
        $moisdebut= date_format($startmois,'M');
        $moistoday=date_format($todaydate, 'M');
        //print_r($emp->getDatedebut());
        if(strcmp($moisdebut,$moistoday)==0){
            $anniversaire=true;
        }else{
            $anniversaire=false;
        }
