...
class viewTrainingListAction extends baseLeaveAction {
   ...
   // protected $leavePeriodService;
    ...
    public function getTrainingRequestService() {
        if (is_null($this->trainingRequestService)) {
            $trainingRequestService = new TrainingRequestService();
            $trainingRequestService->setTrainingRequestDao(new TrainingRequestDao());
     ...
     * @return void
     */
    public function setTrainingRequestService(TrainingRequestService $trainingRequestService) {
        $this->trainingRequestService = $trainingRequestService;
    }
    ...
    public function execute($request) {
        ...
        $this->baseUrl = $mode == TrainingListForm::MODE_MY_TRAINING_LIST ? 'training/viewMyTrainingList' : 'training/viewTrainingList';

        $this->_setPage($mode, $page);

        if ($this->trainingListPermissions->canRead()) {
        ...
        $this->setTemplate('viewTrainingList');
    }
    ...
    protected function getTrainingListForm($mode) {
        $this->form = new TrainingListForm($mode);
        return $this->form;
    }
    ...
    protected function getEmployeeFilter($mode, $empNumber) {
        ...
        if ($mode == TrainingListForm::MODE_MY_TRAINING_LIST) {

            $employeeFilter = $loggedInEmpNumber;
        } else {
            $manager = $this->getContext()->getUserRoleManager();
            $requiredPermissions = array(
                BasicUserRoleManager::PERMISSION_TYPE_ACTION => array('view_training_list'));
    ...
    protected function _setPage($mode, $page) {
        $this->getUser()->setAttribute($mode . '.page', $page, 'training_list');
    }
    ...
    protected function _getPage($mode) {
        return $this->getUser()->getAttribute($mode . '.page', 1, 'training_list');
    }
    ...
    protected function _setFilters($mode, array $filters) {
        return $this->getUser()->setAttribute($mode . '.filters', $filters, 'training_list');
    }
    ...
    protected function _getFilters($mode) {
        return $this->getUser()->getAttribute($mode . '.filters', null, 'training_list');
    }
    ...
    protected function getPermissions(){
        return $this->getDataGroupPermissions('training_list', false);
    }
